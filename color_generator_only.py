...
# @Date:   25-Aug-2020 08:08:12:120  GMT-0600
# @Email:  rijshouray@gmail.com
# @Filename: color_generator_only.py
# @Last modified by:   Ray
# @Last modified time: 24-Feb-2021 01:02:95:956  GMT-0700
# @License: [Private IP]
...
############################################
# MASTER VARIABLES ##########
# SORRY FOR THE BILLION COLOURS YOU'RE ABOUT TO SEE"""
...
def plot3D_object(figure_size, title, labels, limits, df, color_map, cyl, keys):
    ...
    if cyl:
        Xc, Yc, Zc = data_for_cylinder_along_z((limits[0][0] + limits[0][1]) / 2.0,
                                               (limits[1][0] + limits[1][1]) / 2.0, min(limits[0][0],
                                                                                        limits[0][1],
                                                                                        limits[1][0],
                                                                                        limits[1][1]),
                                               limits[2][1])
...
def color_location(df_annotated_RGB, color_3_tuple):
    ...
    return list(chain.from_iterable(df_annotated_RGB.loc[(round(df_annotated_RGB['R']) == round(color_3_tuple[0])) &
                                                         (round(df_annotated_RGB['G']) == round(color_3_tuple[1])) &
                                                         (round(df_annotated_RGB['B']) == round(color_3_tuple[2]))
                                                         ].drop_duplicates(keep='first').to_numpy().tolist()))[3]
...
display_inv_transform_sRGB('LAB-PCA-sRGB', pca_LAB, kmeans_PCA_LAB, X_Fit_CIELAB_to_PCA, y_kmeans_PCA_LAB,
                           RGB_values, plot3D_object(GLOBAL_FIG_SIZE,
                                                     'Coolors sRGB Gamut in sRGB Gamut (w/ LAB-PCA ' +
                                                     'Centroid Connections)', ('R', 'G', 'B'),
                                                     ((0, 255), (0, 255), (0, 255)), df_RGB, flat_HEX, False, None))


def display_inv_transform_sRGB(pipeline_str, pca_, kmeans_PCA, X_Fit_Gamut_to_PCA, y_kmeans_PCA, RGB_values,
                               sRGB_plot_ORIGINAL):
    ...
    if(pre_PCA_dimension == 'sRGB'):
        ...
        # Convert from LAB/LCH -> sRGB (CIE LAB/LCH -> sRGB). Shouldn't be any overlap
        #  between CIE LAB/LCH to sRGB (never traversed beyond sRGB Gamut).
    ...
    ax_Gamut_inverse_transform_CENTROID_PLOTS.scatter(df_Gamut_pca_sRGB['R'], df_Gamut_pca_sRGB['G'],
                                                      df_Gamut_pca_sRGB['B'],  c=df_Gamut_pca_sRGB['Category'],
                                                      cmap='viridis', edgecolor='k', s=60, alpha=0.5)
    ax_Gamut_inverse_transform_CENTROID_PLOTS.scatter(df_Gamut_pca_sRGB_CENT['R'], df_Gamut_pca_sRGB_CENT['G'],
                                                      df_Gamut_pca_sRGB_CENT['B'], s=300, c='r', marker='*',
                                                      label='Centroid')
...
def graph_from_weights(annotated_df):
    ...
    complete_EDGE_LIST = [tuple(sorted(tup))
                          for tup in list(chain.from_iterable([w.edges for w in test_graphs]))]
    edge_weights = collections.Counter(complete_EDGE_LIST)
    first_edge_value, second_edge_value = [i[0]
                                           for i in list(edge_weights)], [i[1] for i in list(edge_weights)]
...
def compare_RGB(obj1, obj2):
    return (round(obj1.rgb_r) == round(obj2.rgb_r) and
            round(obj1.rgb_g) == round(obj2.rgb_g) and
            round(obj1.rgb_b) == round(obj2.rgb_g))


def compare_LAB(obj1, obj2):
    return (round(obj1.lab_l) == round(obj2.lab_l) and
            round(obj1.lab_a) == round(obj2.lab_a) and
            round(obj1.lab_b) == round(obj2.lab_b))


def compare_LCH(obj1, obj2):
    return (round(obj1.lch_l) == round(obj2.lch_l) and
            round(obj1.lch_c) == round(obj2.lch_c) and
            round(obj1.lch_h) == round(obj2.lch_h))
...
def remove_occurences_LAB(base, comparer):
    ...
    for item in base:
        for element in comparer:
            if(compare_LAB(item, element) is True):
    ...
    for pal_i in range(len(lop)):
        for color_i in range(len(lop[pal_i])):
            if(init_type is sRGBColor):
                gamut_to_lab = convert_color(sRGBColor(lop[pal_i][color_i][0], lop[pal_i][color_i][1],
                                                       lop[pal_i][color_i][2]), LabColor, target_illuminant='d50')
            elif(init_type is LCHabColor):
                gamut_to_lab = convert_color(LCHabColor(lop[pal_i][color_i][0], lop[pal_i][color_i][1],
                                                        lop[pal_i][color_i][2]), LabColor, target_illuminant='d50')
...
try:
    ...
    print('1 - Adobe Scraping: Page timed out after 10 secs. ' +
          'Color-containing DIVS are not showing up for some reason.')
...
for page_num in range(ADOBE_FULL_SCROLL_NUMBER):
    ...
    try:
        ...
        print('2 - Adobe Scraping: Page timed out after 10 secs. ' +
              'Color-containing DIVS are not showing up for some reason.')
...
ADOBE_HEX_values_o = list(set([tuple([colormap.rgb2hex(int(color_str[0]), int(color_str[1]), int(color_str[2])
                                                       ).replace('#', "")
                                      for color_str in re.findall(pattern_ADOBE,
                                                                  ALL_ADOBE_DIVS[i].replace(", ", ","))])
                               for i in range(num_ADOBE_palettes)]))
...
RGB_values_o = [tuple([HEX_TO_RGB(HEX_CODE, clamped=True) for HEX_CODE in HEX_PALETTE])
                for HEX_PALETTE in HEX_values_o]
# Convert from RGB -> RGB (sRGB [-> CIE XYZ] -> CIE LAB). No overlap between sRGB to CIE LAB directly, but CIE XYZ???.
LAB_values_o = [tuple([convert_color(sRGBColor(RGB_CODE[0], RGB_CODE[1], RGB_CODE[2]), LabColor,
                                     target_illuminant='d50').get_value_tuple() for RGB_CODE in RGB_PALETTE])
                for RGB_PALETTE in RGB_values_o]
# Convert from LAB -> LCH (CIE LAB -> CIE LCH). No overlap between CIE LAB to CIE LCH.
LCH_values_o = [tuple([convert_color(sRGBColor(RGB_CODE[0], RGB_CODE[1], RGB_CODE[2]), LCHabColor,
                                     target_illuminant='d50').get_value_tuple() for RGB_CODE in RGB_PALETTE])
                for RGB_PALETTE in RGB_values_o]
...
# format = [str(color) for color in [[round(r * 255), round(g * 255), round(b * 255)]
#                                    for r, g, b in incomplete_list]]
#     final_given = ''
#     final_N = ''
...
ax_RGB_inverse_transform_CENTROID_PLOTS, df_RGB_pca_sRGB, sRGB_plot_ORIGINAL = display_inv_transform_sRGB(
    'sRGB-PCA-sRGB', pca_sRGB, kmeans_PCA_sRGB, X_Fit_sRGB_to_PCA, y_kmeans_PCA_sRGB, RGB_values, plot3D_object(
        GLOBAL_FIG_SIZE, 'Coolors sRGB Gamut in sRGB Gamut (w/ sRGB-PCA Centroid Connections)',
        ('R', 'G', 'B'), ((0, 255), (0, 255), (0, 255)), df_RGB, flat_RBG, False, None))
ax_LAB_inverse_transform_CENTROID_PLOTS, df_LAB_pca_sRGB, sRGB_plot_ORIGINAL = display_inv_transform_sRGB(
    'LAB-PCA-sRGB', pca_LAB, kmeans_PCA_LAB, X_Fit_CIELAB_to_PCA, y_kmeans_PCA_LAB, RGB_values, plot3D_object(
        GLOBAL_FIG_SIZE, 'Coolors sRGB Gamut in sRGB Gamut (w/ LAB-PCA Centroid Connections)',
        ('R', 'G', 'B'), ((0, 255), (0, 255), (0, 255)), df_RGB, flat_RBG, False, None))
ax_LCH_inverse_transform_CENTROID_PLOTS, df_LCH_pca_sRGB, sRGB_plot_ORIGINAL = display_inv_transform_sRGB(
    'LCH-PCA-sRGB', pca_LCH, kmeans_PCA_LCH, X_Fit_CIELCH_to_PCA, y_kmeans_PCA_LCH, RGB_values, plot3D_object(
        GLOBAL_FIG_SIZE, 'Coolors sRGB Gamut in sRGB Gamut (w/ LCH-PCA Centroid Connections)',
        ('R', 'G', 'B'), ((0, 255), (0, 255), (0, 255)), df_RGB, flat_RBG, False, None))
#########################################################
#########################################################
# Annotate binned HEX List with group numbers
ALL_RGB_from_sRGB_annotations = [
    [color_location(df_RGB_pca_sRGB, [i * 255.0 for i in color]) for color in palette] for palette in RGB_values]
...
# populate_files_ML(OPEN_TAB, ALL_RGB_from_sRGB_annotations, K, split_threshold, VIS_parameters,
#                   shape_name, VIS_location, prim_dir_name='sRGB-sRGB-PCA-sRGB/')
# populate_files_ML(OPEN_TAB, ALL_RGB_from_LAB_annotations, K, split_threshold, VIS_parameters,
#                   shape_name, VIS_location, prim_dir_name='sRGB-LAB-PCA-sRGB/')
# populate_files_ML(OPEN_TAB, ALL_RGB_from_LCH_annotations, K, split_threshold, VIS_parameters,
#                   shape_name, VIS_location, prim_dir_name='sRGB-LCH-PCA-sRGB/')
...
for palette_num in range(NUM_PALETTES):
    ...
    for color in start_pick:
        ...
        centroid_color_OBJ = LabColor(LAB_CENTROIDS[centroid][0],
                                      LAB_CENTROIDS[centroid][1], LAB_CENTROIDS[centroid][2])
        ...
        current_color_LAB_OBJ = convert_color(
            sRGBColor(color[0], color[1], color[2]), LabColor, target_illuminant='d50')
    ...
    while(traverse is True):
        ...
        centroid_color_OBJ = LabColor(LAB_CENTROIDS[current_cluster][0],
                                      LAB_CENTROIDS[current_cluster][1], LAB_CENTROIDS[current_cluster][2])
        # Determine all colors in centroid
        cluster_colors = np.asarray(df_LAB_pca_sRGB.loc[df_LAB_pca_sRGB['Category'] == current_cluster
                                                        ].reset_index().drop(['index', 'Category'], 1))
        # Configure LabColor object for all Centroid-specific cluster colors
        LAB_LIST_OBJ = [convert_color(sRGBColor(RGB_CODE[0] / 255.0, RGB_CODE[1] / 255.0,
                                                RGB_CODE[2] / 255.0), LabColor, target_illuminant='d50')
                        for RGB_CODE in cluster_colors]
        # Filter LIST_OBJ (all the colors in the specific centroid) to not include previously
        #  chosen colors (non-replacement)
        # Unintended (but not harmful) consequence: may delete all repeats, even if same color is
        #  repeated in LIST_OBJ
        LAB_LIST_OBJ = remove_occurences_LAB(LAB_LIST_OBJ, LAB_color_map)
        if(len(LAB_LIST_OBJ) == 0):
            print('TRAVERSAL CHECKPOINT: No more data in cluster ' + str(current_cluster) +
                  ' to work with. outer_iteration: ' + str(outer_iteration))
        ...
        # neighbors = sorted([(node, len(set(list(LAB_G.neighbors(node))))) for node in LAB_G],
        #                    key = lambda tup: tup[0])[int(current_cluster)][1]
        # Determine weights for each edge with chosen node
        connected_clusters = [tup[1] for tup in list(LAB_G.edges(int(current_cluster)))]
        connected_weights = sorted([(connected_cluster, LAB_G[int(current_cluster)][connected_cluster]['weight'])
                                    for connected_cluster in connected_clusters], key=lambda tup: tup[1], reverse=True)
        ...
        if(CENTROID_PICK_METHOD == 'MAX'):
            ...
            historical_clusters = dict(collections.Counter(
                [cluster for d_color, cluster, d_iteration in LAB_color_map_detailed]))
            # Decide if self-loop should be excluded (add gradient hyperparameter?)
            if(DIVERSITY):
                # Check if there's a self loop and delete current_cluster from possible options if
                #  been accessed over MAX_CENTROID_ACCESS times
                # there is a self-loop, remove self-loop and any future option with > 3
                #  traversals (consec. or non-consec.)
                try:
                    tuple_of_current_cluster_connection = [
                        relation for relation in connected_weights if relation[0] == current_cluster][0]
                    if(historical_clusters[current_cluster] >= MAX_CENTROID_ACCESS):
                        ...
                        proposed_next_cluster_tuple = [relation for relation in connected_weights
                                                       if relation[0] == proposed_next_cluster][0]  # expected tuple
                        try:
                            # if proposed_next_cluster access > MAX_CENTROID_ACCESS
                            while(historical_clusters[proposed_next_cluster] > MAX_CENTROID_ACCESS):
                                # remove from candidates
                                connected_weights.remove(proposed_next_cluster_tuple)
                                # re-assign proposed next cluster
                                proposed_next_cluster = int(connected_weights[0][0])
                        except Exception:
                            # proposed_next_cluster is not in collections, meaning it hasn't been accessed
                            #  (it will be set as the next_cluster)
                            pass
                except IndexError:  # there isn't a self-loop
                    proposed_next_cluster = int(connected_weights[0][0])                # Next expected color
                    proposed_next_cluster_tuple = [
                        relation for relation in connected_weights if relation[0] == proposed_next_cluster][0]
                    # expected tuple
                    try:
                        # if proposed_next_cluster access > MAX_CENTROID_ACCESS
                        while(historical_clusters[proposed_next_cluster] > MAX_CENTROID_ACCESS):
                        ...
                        #  (it will be set as the next_cluster)
                        pass
            # Decide what other centroids should be excluded to prevent oscillation among n-few centroids
            #  (analyze historical data)
            # Assign/set node with maximum weight as next_cluster (could be same as current_centroid
...
LAB_RGB_color_maps = [[convert_color(LAB_CODE, sRGBColor, target_illuminant='d50').get_value_tuple()
                       for LAB_CODE in LAB_color_map] for LAB_color_map in LAB_color_maps]
